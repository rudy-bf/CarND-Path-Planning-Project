        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			for (int i = 0; i < sensor_fusion.size(); i++) {
				// car is in what lane?
				// [i] is the nth car
				// [6] is the `d` value in Frenet coorindates
			...
			// Later we will interpolate these waypoints w/ a spline and fill it in with more points that control spaced waypoints
			vector<double> ptsx;
			vector<double> ptsy;

			// reference x, y, yaw states
			// either we will reference the starting point as wehre the car is or at the previous path's end point
			double ref_x = car_x;
			double ref_y = car_y;
			...
			if (prev_size < 2) {
				cout << "Previous Size is to SMALL." << endl;
				...
				cout << "We have sufficient previous pass points." << endl;
				...
				// User two points that make the path tangent to the prev's path's end point
			...
			// Note: 
			//  - So instead of looking out one distance incremeent, we are looking 30, 60, 90
			//	- Instead of looping and creating 50 pts, we are creating these 3.
			//	- Instead of 0.5 meters spaced, they are all the way to 30
			...
			// Thus far, Vectors of Points Now Have:
			//	- 2 Previous Points 
			//	- Location of car at 30, 60, 90

			// Transformation
			for (int i = 0; i < ptsx.size(); i++ ){

				// shift car reference angle to 0 degree -> Recall MPC
			...
			// set (x, y) points to the spline -- "Anchor Points" to Spline
			...
			// "Next X Vals" are the Future Path
			for (int i = 0; i < previous_path_x.size(); i++) {
				// Loaded up the future path w/ whatever was left of prev path
			...
			for (int i = 1; i <= 50 - previous_path_x.size(); i++) {
				// 2.24 -> meters per second not miles per hour
				...
				// Youtube - Time: 38:40
				// rorate back to normal after rotating it earlier
